                           ...
                           void *buf, size_t len)
{
 ...
 retry_read:
    ret = tls_ctx->backend->read(u_conn, buf, len);
    if (ret == FLB_TLS_WANT_READ) {
        u_conn->thread = th;
...
retry_write:
    ret = tls_ctx->backend->write(&u_conn->tls_session->ssl,
                           ...
                           struct flb_thread *th)
{
    ...
    /* Create TLS session */
    session = tls_ctx->backend->session_create(tls);
    if (!session) {
    ...
    /* Store session and net context fd */
    u_conn->tls_session = session;


 retry_handshake:
    ret = flb_net_handshake(tls, session);
    if (ret != 0) {
...
{
    return tls->api->session_destroy(u_conn->tls_session);
}
